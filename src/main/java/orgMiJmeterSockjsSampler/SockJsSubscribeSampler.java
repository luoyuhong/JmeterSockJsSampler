...
/* 		for (int i = 0; i < splitHeaders.length; i++) {
		...
		try {
			...
			StompSession.Subscription subscription = stompSession.subscribe(
					context.getParameter(STOMP_SUBSCRIBE_ENDPOINT), sessionHandler);
			while (true) {
				//StompHeaders stompHeaders1 = subscription.getSubscriptionHeaders();
				//logger.info(subscription.getSubscriptionHeaders());
				Thread.sleep(10000);
			}
		} catch (Exception e) {
			e.printStackTrace();
